 ...
 *
 * This constant represents the smallest prime number, which is 2. It is used in prime number calculations
 * to handle special cases and optimizations. Since 2 is the only even prime number, it often requires
 * separate handling in algorithms related to prime numbers.
 */
private const val MIN_PRIME = 2

/**
 * The first odd number to check for divisibility in prime number calculations.
 *
 * This constant is used in the context of determining prime numbers. When checking if a number `n` is prime,
 * trial division starts with this odd number, which is 3. As even numbers other than 2 are not prime,
 * the trial division process can skip even numbers, starting from this first odd number after 2.
 * This optimization reduces the number of checks needed to determine if a number is prime.
 */
private const val FIRST_ODD_DIVISOR = 3
...
private fun isPrime(n: Int): Boolean {
    ...
    when {
        n <= 1 -> isPrime = false
        n != MIN_PRIME && n % 2 == 0 -> isPrime = false
        else -> {
            val sqrtN = sqrt(n.toDouble()).toInt()
            for (i in FIRST_ODD_DIVISOR..sqrtN step 2) {
                if (n % i == 0) {
                    isPrime = false
                    break
                }
