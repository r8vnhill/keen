...
fun Random.nextIntInRange(range: ClosedRange<Int>): Int {
    constrained {
        "Cannot generate a random integer within an empty range" { constraint { !range.isEmpty() } }
    }.onLeft { throw it }
...
fun <T> Random.subsets(
    elements: List<T>,
    size: Int,
    exclusive: Exclusivity = Exclusivity.NON_EXCLUSIVE,
    ...
    val remainingElements = elements.toMutableList().also {
        it.shuffle(this)
    }
    var i = 0
    ...
    while (remainingElements.isNotEmpty() && i < limit) {
        when (exclusive) {
            Exclusivity.NON_EXCLUSIVE -> {
                // If non-exclusive, creates a subset of the given size.
            ...
            Exclusivity.EXCLUSIVE -> {
                ...
                subsets += remainingElements.take(size)
            }
        }
        i++
    }
 ...
 */
private fun <T> validateSubsetsInput(elements: List<T>, size: Int, exclusive: Exclusivity, limit: Int) =
    constrained {
        if (elements.isEmpty()) {
            ...
            if (exclusive == Exclusivity.EXCLUSIVE) {
