    ...
    "Can be created with" - {
        "a fitness function and a genotype factory" {
            checkAll(arbGenotypeFactory(Arb.list(arbDoubleChromosomeFactory()))) { genotypeFactory ->
                ...
                with(GeneticAlgorithmFactory(fitnessFunction, genotypeFactory)) {
                    this.genotypeFactory shouldBe genotypeFactory
                    this.fitnessFunction shouldBe fitnessFunction
                    populationSize shouldBe GeneticAlgorithmFactory.DEFAULT_POPULATION_SIZE
                    survivalRate shouldBe GeneticAlgorithmFactory.DEFAULT_SURVIVAL_RATE
                    parentSelector shouldBe GeneticAlgorithmFactory.defaultParentSelector()
                    survivorSelector shouldBe GeneticAlgorithmFactory.defaultSurvivorSelector()
                    alterers shouldBe GeneticAlgorithmFactory.defaultAlterers()
                    limits shouldBe GeneticAlgorithmFactory.defaultLimits()
                    ranker shouldBe GeneticAlgorithmFactory.defaultRanker()
                    listeners shouldBe GeneticAlgorithmFactory.defaultListeners()
                    evaluator shouldBe GeneticAlgorithmFactory.defaultEvaluator()
                    interceptor shouldBe GeneticAlgorithmFactory.defaultInterceptor()
                }
            }
        }
    }
    ...
    "Setting the population size" - {
        "to a positive value mutates the property" {
            ...
            checkAll(Arb.positiveInt(), arbGenotypeFactory) { size, genotypeFactory ->
                GeneticAlgorithmFactory({ _: Genotype<Double, DoubleGene> -> 0.0 }, genotypeFactory).apply {
                    populationSize = size
                }
            }
        }
