 ...
 *
 * `GeneticOperator` is an interface representing operations applied to an [GeneticEvolutionState] in the context of
 ...
 *
 * ## Key Concepts:
 * - **Evolution State Manipulation**: Each genetic operator works with an [GeneticEvolutionState], which represents
 *   the current state of the evolutionary process, including the population and generation number.
 * - **Output Size Control**: Operators are responsible for producing a new [GeneticEvolutionState] with a specified
 ...
 * [GeneticEvolutionState] and returns a new state representing the next generation.
 *
...
interface GeneticOperator<T, G> where G : Gene<T, G> {
    ...
    /**
     * Applies the genetic operation to the provided [GeneticEvolutionState] and returns a new state.
     *
     ...
     * @return A new [GeneticEvolutionState] resulting from applying the genetic operation.
     */
    operator fun invoke(state: GeneticEvolutionState<T, G>, outputSize: Int): GeneticEvolutionState<T, G>
}
