...
fun `test Engine single evolution step`() = freeSpec {
    "An Evolution Engine can evolve a population from a given state" - {
        "when the state is empty" - {
            ...
            "should return a new state with the expected individuals" {
                ...
                val result = engine.evolve(state)
                result shouldBe EvolutionState(1, Individual<Int, IntGene>(Genotype(), 1.0), Individual(Genotype(), 1.0), Individual(Genotype(), 1.0), Individual(Genotype(), 1.0))
            }
        }
    }
}
