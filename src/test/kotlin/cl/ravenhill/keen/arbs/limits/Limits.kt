 ...
 */
fun <T, G> Arb.Companion.generationCount(count: Arb<Int> = int(1..100)) where G : Gene<T, G> =
    arbitrary {
 ...
 */
fun <T, G> Arb.Companion.generationCount(count: Int): Arb<GenerationCount<T, G>>
        where G : Gene<T, G> = generationCount(constant(count))
 ...
 */
fun <T, G> Arb.Companion.steadyGenerations(steady: Arb<Int> = int(1..100)) where G : Gene<T, G> =
    arbitrary {
...
/**
 * Generates a [SteadyGenerations] limit with a fixed number of steady generations for property-based testing.
 *
 * This variant of the `steadyGenerations` function creates a [SteadyGenerations] instance with a specified
 * constant number of steady generations. It is useful in scenarios where a fixed number of generations
 * without improvement in fitness is required to trigger the limit in a genetic algorithm.
 *
 * The function simplifies the creation of [SteadyGenerations] limits for cases where the number of steady
 * generations is predetermined and does not require arbitrary generation.
 *
 * @param steady A fixed number of steady generations required to satisfy the limit.
 *
 * @return A [SteadyGenerations] instance with the specified number of steady generations.
 *
 * @param T The type representing the genetic data or information.
 * @param G The type of [Gene] associated with the genetic data.
 */
fun <T, G> Arb.Companion.steadyGenerations(steady: Int) where G : Gene<T, G> =
    steadyGenerations<T, G>(constant(steady))
