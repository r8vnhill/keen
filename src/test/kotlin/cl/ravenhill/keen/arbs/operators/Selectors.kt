 ...
 */
fun <T, G> Arb.Companion.tournamentSelector(sampleSize: Arb<Int> = int(1..5)) where G : Gene<T, G> =
    arbitrary {
 ...
 */
fun <T, G> Arb.Companion.rouletteWheelSelector(sorted: Arb<Boolean> = boolean()) where G : Gene<T, G> =
    arbitrary {
 ...
 */
fun <T, G> Arb.Companion.selector(): Arb<Selector<T, G>> where G : Gene<T, G> =
    choice(randomSelector(), tournamentSelector(), rouletteWheelSelector())
